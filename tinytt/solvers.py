        ...
        if self.prec is None or not apply_prec:
            ...
            if self.band_diagonal >= 0:
                ...
                for i in range(-self.band_diagonal, self.band_diagonal + 1):
                    ...
                    if i < 0:
                        tmp = _pad_like_torch(
                            tmp[:, :, :i, :], (0, 0, -i, 0, 0, 0, 0, 0)
                        )
                    else:
                        tmp = _pad_like_torch(
                            tmp[:, :, i:, :], (0, 0, 0, i, 0, 0, 0, 0)
                        )
                    w += tmp
...
def _amen_mm_python(
    ...
    for swp in range(nswp):
        ...
        for k in range(d - 1, 0, -1):
            ...
            if not last:
                Phiz[k] = (
                    _compute_phi_bck_x(Phiz[k + 1], z_cores[k], x_cores[k])
                    / normA[k - 1]
                )
                Phiz_rhs[k] = (
                    _compute_phi_bck_AB(
        ...
        for k in range(d):
            ...
            solution_now = (
                _local_AB(Phis_rhs[k], Phis_rhs[k + 1], A_cores[k], B_cores[k]) * nrmsc
            )
            ...
            if k < d - 1:
                ...
                if not last:
                    Phiz[k + 1] = (
                        _compute_phi_fwd_x(Phiz[k], z_cores[k], x_cores[k]) / normA[k]
                    )
                    Phiz_rhs[k + 1] = (
                        _compute_phi_fwd_AB(
...
def _amen_solve_python(
    ...
    for swp in range(nswp):
        ...
        for k in range(d - 1, 0, -1):
            ...
            if not last:
                Phiz[k] = (
                    _compute_phi_bck_A(Phiz[k + 1], z_cores[k], A.cores[k], x_cores[k])
                    / normA[k - 1]
                )
                Phiz_b[k] = (
                    _compute_phi_bck_rhs(Phiz_b[k + 1], b.cores[k], z_cores[k])
                    / normb[k - 1]
                )
        ...
        for k in range(d):
            ...
            if use_full:
                if verbose:
                    print(
                        "\t\tChoosing direct solver (local size %d)...."
                        % (rx[k] * N[k] * rx[k + 1])
                    )
            ...
            if k < d - 1:
                ...
                if not last:
                    Phiz[k + 1] = (
                        _compute_phi_fwd_A(Phiz[k], z_cores[k], A.cores[k], x_cores[k])
                        / normA[k]
                    )
                    Phiz_b[k + 1] = (
                        _compute_phi_fwd_rhs(Phiz_b[k], b.cores[k], z_cores[k])
                        / normb[k]
                    )
            else:
                x_cores[k] = tn.reshape(
