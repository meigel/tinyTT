...
def test_gpu_helpers_and_linalg():
    ...
    y = tt.ones([2, 3, 4], device=DEVICE)

    _assert_tt_device(x)
    _assert_tt_device(y)

    z = x + y
    assert np.allclose(z.full().numpy(), full + 1.0, atol=1e-10)

    div = tt.elementwise_divide(x, y + 1.0)
    assert np.allclose(div.full().numpy(), full / 2.0, atol=1e-10)

    dot_val = tt.dot(x, x).numpy().item()
    assert np.isclose(dot_val, np.sum(full * full), rtol=1e-10, atol=1e-10)
...
def test_gpu_fast_products():
    ...
    y = tt.random([2, 2, 2], [1, 2, 2, 1], device=DEVICE)

    z = tt.fast_hadammard(x, y, eps=1e-8)
    ref = x.full().numpy() * y.full().numpy()
    assert np.allclose(z.full().numpy(), ref, atol=1e-6)

    A = tt.random([(2, 2), (2, 2), (2, 2)], [1, 2, 2, 1], device=DEVICE)
    mv = tt.fast_mv(A, x, eps=1e-8)

    A_full = A.full().numpy().reshape(8, 8)
    x_full = x.full().numpy().reshape(-1)
    mv_ref = A_full @ x_full
    assert np.allclose(mv.full().numpy().reshape(-1), mv_ref, atol=1e-6)

    mm = tt.fast_mm(A, A, eps=1e-8)
    mm_ref = A_full @ A_full
    assert np.allclose(mm.full().numpy().reshape(8, 8), mm_ref, atol=1e-6)
...
def test_gpu_matvec_and_fast_matvec():
    ...
    A_full = rng.rand(4, 4).astype(np.float64)
    x_full = rng.rand(4).astype(np.float64)

    A = tt.TT(A_full.reshape(2, 2, 2, 2), shape=[(2, 2), (2, 2)], eps=1e-12, device=DEVICE)
    x = tt.TT(x_full.reshape(2, 2), eps=1e-12, device=DEVICE)
    ...
    assert np.allclose(y_fast.full().numpy().reshape(-1), ref, atol=1e-6)


def test_gpu_ttm_multiply_and_autograd():
    rng = np.random.RandomState(1)
    A_full = rng.rand(4, 4).astype(np.float64)
    B_full = rng.rand(4, 4).astype(np.float64)

    A = tt.TT(A_full.reshape(2, 2, 2, 2), shape=[(2, 2), (2, 2)], eps=1e-12, device=DEVICE)
    B = tt.TT(B_full.reshape(2, 2, 2, 2), shape=[(2, 2), (2, 2)], eps=1e-12, device=DEVICE)

    C = A @ B
    ref = A_full @ B_full
    assert np.allclose(C.full().numpy().reshape(4, 4), ref, atol=1e-10)

    x_full = rng.rand(2, 3, 2).astype(np.float64)
    x = tt.TT(x_full, eps=1e-12, device=DEVICE)
    tt.grad.watch(x)
    val = tt.dot(x, x)
