...
class TestLinearTTMap:
    ...
    def test_full_training_loop_does_not_diverge(self):
        """End-to-end training: final loss must be lower than initial loss and
        stay finite. test_training_step above only checks one step on one
        point; this guards against a misconfigured loop that diverges over
        many steps (the regression the ctt_ode_example.png originally hit)."""
        rng = np.random.RandomState(0)
        n = 80
        d = p = 2
        a = rng.randn(n, d)
        mu = rng.uniform(-1.0, 1.0, (n, p))
        # Linear-in-(a, mu) target that LinearTTMap *can* represent exactly.
